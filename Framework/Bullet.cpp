...
Bullet::Bullet(const wchar_t* path, float speed, float speedRate, float angle, float angleRate, float damage)
	: GameObject(path), speed(speed), speedRate(speedRate), angle(angle), angleRate(angleRate), damage(damage) {
	this->speed = speed;
	col = new CircleCollider(*transform, renderer->GetWidth() * 0.5f);	//여기
}

Bullet::~Bullet() {}

void Bullet::Update() { //매 프레임 실행될 함수
	Move();
	CheckOutOfScreen();
}

void Bullet::Move() { //움직일 때 사용할 함수
...
void Bullet::Destroy() {} //삭제될 때 실행할 함수
