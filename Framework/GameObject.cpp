...
GameObject::GameObject(Vector2 position, float angle, Vector2 scale)
	: transform(new Transform(position, angle, scale)), renderer(new Renderer()), isActive(true) {}
...
GameObject::GameObject(Renderer* renderer, Vector2 position, float angle, Vector2 scale)
	: transform(new Transform(position, angle, scale)), renderer(renderer), isActive(true) {}
