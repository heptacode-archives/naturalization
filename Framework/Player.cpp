...
Player::Player(const wchar_t* path, BulletManager* bm) : GameObject(path), bm(bm) {
	moveSpeed = 3.0f;
	//this->bm = bm;
}
void Player::Shoot() {
	Bullet* b = bm->PushBackPlayerBullet(new Bullet(L"bullet.png", 300.0f, 0.0f, 0.75f, 0.00f, 1.0f));
